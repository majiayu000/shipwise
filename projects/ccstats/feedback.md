...
| ccstats launch boundary review | Launch blocker | GitHub release `v0.2.65` did not exist while crates.io was at `0.2.65`; no platform/account authorization or final copy approval was recorded. | Superseded by the 2026-08-13 refresh; release and crate are now aligned at `0.4.0`. |
| ccstats template-flow refresh | Template defect | Real platform copy lived in one hand-written aggregate file instead of exercising `PROJECT_BRIEF -> ANNOUNCEMENT -> platform templates`. | Fixed in the issue #8 implementation branch by adding template-derived files under `projects/ccstats/copy/`. |
| ccstats discoverability refresh | Gate blocker | The local SEO Agent Suite checkout does not expose the documented `repo_seo_baseline.py --project-yaml` capability. | Update SEO Agent Suite to a supporting revision, then rerun the documented project-yaml gate. |
